...
export function createTask(task: Omit<Task, 'id' | 'created_at'>): Task {
  const stmt = db.prepare(`
    INSERT INTO tasks (goal_id, title, description, ai_suggestion, tags, status, priority, due_date, completed_at, assignee, executor, progress, result)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
...
export function updateTask(id: number, fields: Partial<Task>): Task | undefined {
  const allowed = ['goal_id', 'title', 'description', 'ai_suggestion', 'tags', 'status', 'priority', 'due_date', 'completed_at', 'assignee', 'executor', 'progress', 'result'];
